...
const THEME_KEY = 'temaControleDormentes';
...
const PERFIS_USUARIO = ['admin', 'fiscalizacao', 'consulta'];
...
const uid = () => (globalThis.crypto?.randomUUID ? globalThis.crypto.randomUUID() : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`);
const todayIso = () => new Date().toISOString().slice(0, 10);
const matches = (haystack, query) => !query || norm(haystack).includes(norm(query));

function perfilAtualNome() {
  return window.Auth?.permissoesAtuais?.().perfil || String(window.USUARIO_ATUAL?.perfil?.perfil || 'consulta').toLowerCase();
}
...
function isAdmin() {
  return window.Auth?.pode?.('gerenciarSistema') || perfilAtualNome() === 'admin';
}
function canWrite() {
  return window.Auth?.pode?.('criar') || window.Auth?.pode?.('editar') || ['admin', 'fiscalizacao'].includes(perfilAtualNome());
}
function canDelete() {
  return window.Auth?.pode?.('excluir') || isAdmin();
}
function perfilLabel(perfil) {
  const p = window.Auth?.normalizarPerfil?.(perfil) || String(perfil || '').toLowerCase();
  if (p === 'admin') return 'ADMIN';
  if (p === 'fiscalizacao') return 'FISCALIZAÇÃO';
  return 'CONSULTA';
}
function perfilBadge(perfil) {
  const p = window.Auth?.normalizarPerfil?.(perfil || 'consulta') || String(perfil || 'consulta').toLowerCase();
  const label = perfilLabel(p);
  const cls = p === 'admin' ? 'badge-reprovado' : p === 'fiscalizacao' ? 'badge-ok' : 'badge-azul';
...
function actionHeader() {
  return (canWrite() || canDelete()) ? '<th>Ações</th>' : '';
}
function actionCell(type, id) {
  if (!canWrite() && !canDelete()) return '';
  const edit = canWrite() ? `<button class="icone-btn" title="Editar" data-edit="${esc(type)}" data-id="${esc(id)}">✎</button>` : '';
  const del = canDelete() ? `<button class="icone-btn del" title="Excluir" data-delete="${esc(type)}" data-id="${esc(id)}">🗑</button>` : '';
  return `<td class="acoes-cel">${edit}${del}</td>`;
}
...
const SubcomponentesApp = {
  ...
  setHeader() {
    ...
    const h1 = document.querySelector('.topo-identidade h1');
    const sub = document.querySelector('.topo-identidade .subtitulo');
  ...
  async refreshData() {
    ...
    try {
      ...
      SubcomponentesApp.toast('Não foi possível atualizar os dados agora.', 'erro');
    } finally {
      if (btn) {
        btn.disabled = false;
  ...
  async init() {
    if (window.Auth?.configurado?.() && window.StoreSubcomponentesSupabase) {
      try {
        ...
        SubcomponentesApp.toast('Não consegui carregar o Supabase. Use a tela Dados para revisar a configuração.', 'erro');
      }
    }
...
function render() {
  SubcomponentesApp.renderNav();
  SubcomponentesApp.setHeader();
  const views = {
  ...
  SubcomponentesApp.applyTheme(document.body.getAttribute('data-tema') || 'claro', false);
}
...
async function saveUsuarioPerfil(data, id) {
  ...
  const perfil = window.Auth?.normalizarPerfil?.(data.perfil || 'consulta') || String(data.perfil || 'consulta').toLowerCase();
...
function exportDashboardExcel() {
  ...
  SubcomponentesApp.toast('Dashboard filtrado exportado para Excel.');
}
function exportDashboardPdf() {
  ...
  if (!win) { SubcomponentesApp.toast('O navegador bloqueou a janela do PDF. Permita pop-ups para este site.', 'erro'); return; }
  win.document.open();
  win.document.write(html);
  win.document.close();
}
async function restoreJson() {
  SubcomponentesApp.toast('Restauração de backup desativada para proteger os dados da empresa.', 'erro');
}
async function reloadSeed() {
  SubcomponentesApp.toast('Recarregamento da base inicial desativado para não substituir dados do Supabase.', 'erro');
}
async function clearAll() {
  SubcomponentesApp.toast('Limpeza total desativada. Exclua registros individualmente pelas telas do sistema.', 'erro');
}
...
async function bootstrap() {
  document.body.classList.add('pagina-subcomponentes');
